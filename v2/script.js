document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('udheyaBooking', () => ({
        ...
        stepSectionsMeta: [],
        countdown: { days: '00', hours: '00', minutes: '00', seconds: '00', ended: false }, // For D:H:M:S
        promoHasEnded: false, // Kept for overall promo status, countdown.ended is more specific to timer
        calculatedPromoDaysLeft: 0, // Still useful for the stepper completion logic maybe, or other simple displays
        ...
        setError(field, typeOrMessage) {
            if (typeof typeOrMessage === 'string') { // It's a type key
                this.errors[field] = this.errorMessages[typeOrMessage] || this.errorMessages.required;
            } else { // It's a custom message object {en: ..., ar: ...}
                this.errors[field] = typeOrMessage;
            }
        },
        clearError(field) { if (this.errors[field]) this.$delete(this.errors, field); }, // Use $delete for reactivity
        clearAllErrors() { this.errors = {}; },
        focusOnRef(refName) { this.$nextTick(() => { if (this.$refs[refName]) { this.$refs[refName].focus(); this.$refs[refName].scrollIntoView({ behavior: 'smooth', block: 'center' }); } }); },

        get _needsDeliveryDetails() { /* ... (same) ... */
        ...
        _getDeliveryLocation(lang) { /* ... (same) ... */
        ...
        get summaryDeliveryToEN() { /* ... (updated for 5-step validation) ... */
        ...
        get summaryDeliveryToAR() { /* ... (updated for 5-step validation) ... */
        ...
        get summaryDistributionEN() { /* ... (same) ... */
        ...
        get summaryDistributionAR() { /* ... (same) ... */
        ...
        async initApp() { /* ... (same as before, but ensure stepSectionsMeta is for 5 steps) ... */
            ...
            this.startOfferDHDMSCountdown(); // Call D:H:M:S version
        ...
        startOfferDHDMSCountdown() { // Renamed for D:H:M:S
            ...
            this.updateDHDMSCountdownDisplay(endDate); // Call D:H:M:S version
            this.countdownTimerInterval = setInterval(() => this.updateDHDMSCountdownDisplay(endDate), 1000);
        },
        updateDHDMSCountdownDisplay(endDate) { // Renamed for D:H:M:S
            ...
            this.calculatedPromoDaysLeft = days; // Keep this simple for other logic if needed
        },

        handleScroll() { /* ... (same as before, but for 5 steps) ... */
            ...
            if (this.currentActiveStep !== newActiveStep) { // Validate before changing step via scroll
                let canScrollToStep = true;
                if (newActiveStep > 1 && !this.currentStepCompleted(1, false)) canScrollToStep = false;
                else if (newActiveStep > 2 && !this.currentStepCompleted(2, false)) canScrollToStep = false;
                else if (newActiveStep > 3 && !this.currentStepCompleted(3, false)) canScrollToStep = false;
                else if (newActiveStep > 4 && !this.currentStepCompleted(4, false)) canScrollToStep = false;
                // No validation for scrolling to step 5, it's just review

                if (canScrollToStep) {
                    this.currentActiveStep = newActiveStep;
                    // this.focusOnRef(this.stepSectionsMeta[newActiveStep - 1]?.titleRef); // Maybe too aggressive on scroll
                }
            }
        },
        updateCities() { /* ... (same) ... */
        ...
        getFormattedPrice(price, currency) { /* ... (same) ... */
        ...
        navigateToStep(targetStep) {
            this.clearAllErrors();
            let canProceed = true;
            // Check if all previous steps are completed before allowing jump
            for (let i = 1; i < targetStep; i++) {
                if (!this.currentStepCompleted(i, false)) { // false = don't show errors for intermediate steps
                    canProceed = false;
                    this.currentActiveStep = i; // Go to the first incomplete step
                    this.validateAndProceedToStep(i + 1, true); // Validate current incomplete step and show its errors
                    break;
                }
            }
            if (canProceed) {
        ...
        currentStepCompleted(stepNumber, showErrors = true) {
            switch (stepNumber) {
                case 1: return this.validateStep1(showErrors);
                case 2: return this.validateStep2(showErrors);
                case 3: return this.validateStep3(showErrors);
                case 4: return this.validateStep4(showErrors);
                case 5: return this.validateStep5(showErrors); // Final submit validation
                default: return false;
            }
        },

        validateAndProceedToStep(nextStep, triggeredByStepper = false) {
            this.clearAllErrors();
            let isValidCurrentStep = false;
            let firstErrorRef = null;

            if (this.currentActiveStep === 1) isValidCurrentStep = this.validateStep1(true);
            else if (this.currentActiveStep === 2) isValidCurrentStep = this.validateStep2(true);
            else if (this.currentActiveStep === 3) isValidCurrentStep = this.validateStep3(true);
            else if (this.currentActiveStep === 4) isValidCurrentStep = this.validateStep4(true);
            // Step 5 is final submission, handled by validateAndSubmitBooking
            ...
            if (isValidCurrentStep) {
                ...
                // Focus has been set by individual validation functions if showErrors was true
                if(!triggeredByStepper) this.scrollToSection(this.stepSectionsMeta[this.currentActiveStep-1].id || '#udheya-booking-form-panel');
            }
        },
        ...
        validateStep1(showErrors = true) {
            ...
            if (!this.selectedAnimal.type) {
                if(showErrors) { this.setError('animal', 'select'); this.focusOnRef(this.selectedAnimal.value === 'baladi' && this.$refs.baladiWeightSelect ? 'baladiWeightSelect' : 'barkiWeightSelect');}
                return false;
            }
            return true;
        },
        ...
        validateStep5(showErrors = true) { // Review & Payment
            ...
            if(!this.paymentMethod) {
                if(showErrors) {this.setError('paymentMethod', 'select'); this.focusOnRef('paymentMethodRadios');}
                return false;
            }
            return true;
        },
        ...
        selectAnimal(cardEl, key) { /* ... (same as before, but navigate to step 2) ... */
        ...
        isLivestockWeightOutOfStock(selEl, key) { /* ... (same) ... */
            const animal = this.productOptions.livestock.find(a => a.value_key === key);
        ...
        updateSelectedPrepStyle(value) { /* ... (same) ... */
        ...
        updateSelectedPackaging(value) { /* ... (same) ... */
        ...
        updateSacrificeDayTexts() { /* ... (same, ensure ID matches HTML: sacrifice_day_select_s3) ... */
        ...
        async validateAndSubmitBooking() {
            ...
            if (!this.currentStepCompleted(5, true)) { this.navigateToStep(5); return; } // Validates payment method
        ...
        async validateAndCheckBookingStatus() { // Renamed for clarity
        ...
        async checkBookingStatus() { /* ... (same logic as before, userFriendlyApiError added) ... */
            ...
            // Validation now happens in validateAndCheckBookingStatus
            try {
        ...
        getSacrificeDayText(val) { /* ... (same, ensure ID matches HTML: sacrifice_day_select_s3) ... */
        ...
        resetAndStartOver() { /* ... (same, ensure all relevant states are reset, focus added) ... */
