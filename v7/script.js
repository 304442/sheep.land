document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('udheyaBooking', () => ({
            ...
            try {
                ...
                if (settingsCollectionData && settingsCollectionData.items && settingsCollectionData.items.length > 0) {
                    ...
                    for (const key in fetchedSettings) {
                        if (fetchedSettings.hasOwnProperty(key) && key !== 'site_name') { // Retained site_name exclusion from your previous script
        ...
        selectAnimal(cardEl, key) { // MODIFIED: Removed auto-advance
            ...
            // User now clicks the "Continue to Customize" button in index.html which calls validateAndProceedToStep(2)
        },
...
(function() {
    ...
    if (getQueryParam(SEEDER_QUERY_PARAM) === 'true') {
        ...
        (async function runInlineSeedActual() { // Renamed for clarity, was runInlineSeed
            // console.log removed
            ...
            // console.logs/warns removed
            if (window.history.replaceState) {
                const cleanURL = window.location.protocol + "//" + window.location.host + window.location.pathname;
                window.history.replaceState({path: cleanURL}, '', cleanURL);
            }
        })(); // Correctly calling the async IIFE
    }
})();
