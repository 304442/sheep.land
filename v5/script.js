document.addEventListener('alpine:init', () => {
    const initialBookingFormState = {
        ...
        errors: {} // Initialized as an empty object
    };
    ...
    async function pbFetch(collection, options = {}) {
        ...
        console.log(`PBFETCH: Requesting ${url}`);
        try {
            const response = await fetch(url, options.fetchOptions);
            console.log(`PBFETCH: Response status for ${url}: ${response.status}`);
            if (!response.ok) {
                ...
                try {
                    ...
                    console.warn(`PBFETCH: Could not parse error response as JSON for ${url}. Body: ${responseBodyForError}`, e);
            ...
            if (!responseBody) { 
                console.log(`PBFETCH: Received empty successful response for ${url}`);
                return { items: [] }; 
            }
            try {
                return JSON.parse(responseBody);
            } catch (e) {
                console.error(`PBFETCH: Could not parse successful response as JSON for ${url}. Body: ${responseBody}`, e);
                throw new Error(`API Error (${collection} ${recordId || params}): Failed to parse successful response. ${e.message}`);
            }
        } catch (networkError) {
    ...
    Alpine.data('udheyaBooking', () => ({
        ...
        setError(field, typeOrMessage) {
            console.log(`setError called for field: ${field}, type/message:`, typeOrMessage);
            if (typeof typeOrMessage === 'string') {
                ...
                if (typeof messageObj?.en === 'string' && typeof messageObj?.ar === 'string') {
                    ...
                    console.warn(`setError: Invalid error message type '${typeOrMessage}' or message object structure for field '${field}'. Defaulting. Message obj:`, messageObj);
                ...
                this.errors[field] = typeOrMessage;
                 ...
                 console.warn(`setError: typeOrMessage is not a recognized string key or a valid message object for field '${field}'. Value:`, typeOrMessage);
                 this.errors[field] = this.errorMessages.required; 
            }
            console.log('Errors object after setError:', JSON.parse(JSON.stringify(this.errors)));
        },
        clearError(field) { console.log(`Clearing error for field: ${field}`); if (this.errors[field]) this.$delete(this.errors, field); },
        clearAllErrors() { console.log('Clearing all errors.'); this.errors = {}; },
        ...
        async initApp() {
            console.log("INITAPP: Alpine component initApp starting...");
            ...
            try {
                ...
                console.log("INITAPP: Fetching app_settings...");
                const settingsCollectionData = await pbFetch('app_settings', { params: settingsParams });
                console.log("INITAPP: app_settings RAW DATA:", settingsCollectionData ? JSON.stringify(settingsCollectionData).substring(0,500) + "..." : "null/undefined");
                ...
                if (settingsCollectionData && settingsCollectionData.items && settingsCollectionData.items.length > 0) {
                    ...
                    console.warn("INITAPP: Global app settings ('global_config') not found in DB. Using defaults. Consider running seeder: index.html?run_db_seed=true");
                }
                console.log("INITAPP: this.appSettings FINAL:", this.appSettings ? JSON.stringify(this.appSettings).substring(0,500) + "..." : "null/undefined" );

                console.log("INITAPP: Fetching livestock_types...");
                const livestockData = await pbFetch('livestock_types'); // Removed sort=created
                console.log("INITAPP: livestock_types RAW DATA:", livestockData ? JSON.stringify(livestockData).substring(0,500) + "..." : "null/undefined");
                ...
                if (livestockData && livestockData.items) {
                    ...
                    console.warn("INITAPP: Livestock data not found or invalid. productOptions.livestock set to [].")
                }
                console.log("INITAPP: this.productOptions.livestock FINAL:", this.productOptions.livestock ? JSON.stringify(this.productOptions.livestock).substring(0,500) + "..." : "null/undefined");

            } catch (error) {
                console.error("INITAPP: Error during initial data fetching:", error);
                ...
                console.log("INITAPP: Exiting try...catch...finally for data fetch.");
            }

            if (typeof this.appSettings.default_currency !== 'string') {
                console.warn('INITAPP: this.appSettings.default_currency is not a string:', this.appSettings.default_currency, 'Setting to EGP.');
                this.appSettings.default_currency = "EGP";
            }
            this.currentCurrency = this.appSettings.default_currency || "EGP";

            console.log("INITAPP: Calling startOfferDHDMSCountdown...");
            this.startOfferDHDMSCountdown();
            console.log("INITAPP: Calling updateSacrificeDayTexts...");
            this.updateSacrificeDayTexts();
            console.log("INITAPP: Calling updateAllDisplayedPrices...");
            this.updateAllDisplayedPrices(); 
            console.log("INITAPP: Calling clearAllErrors...");
            this.clearAllErrors();

            console.log("INITAPP: Setting up $watchers...");
            this.$watch(['selectedAnimal.basePriceEGP', 'selectedPackaging.addonPriceEGP'], () => this.calculateTotalPrice());
            this.$watch('currentCurrency', () => { console.log("WATCH: currentCurrency changed to", this.currentCurrency); this.calculateTotalPrice(); this.updateAllDisplayedPrices(); });
            this.$watch('selectedSacrificeDay.value', () => this.updateSacrificeDayTexts());
            this.$watch('distributionChoice', val => {
                console.log('WATCH: distributionChoice changed to', val);
            ...
            console.log("INITAPP: Setting up $nextTick for scroll and focus...");
            this.$nextTick(() => {
                console.log("INITAPP: Inside $nextTick.");
                this.handleScroll();
                ...
                console.log("INITAPP: $nextTick finished.");
            });
            console.log("INITAPP: Adding scroll listener...");
            ...
            console.log("INITAPP: Finished initApp successfully.");
        },
        ...
        startOfferDHDMSCountdown() {
            ...
            if (!this.appSettings.promo_end_date || typeof this.appSettings.promo_end_date !== 'string') {
                console.warn("COUNTDOWN: promo_end_date is invalid or missing.", this.appSettings.promo_end_date);
                this.countdown.ended = true; return;
            }
            const endDate = new Date(this.appSettings.promo_end_date).getTime();
            if (isNaN(endDate)) {
                console.warn("COUNTDOWN: promo_end_date resulted in an invalid date.", this.appSettings.promo_end_date);
                this.countdown.ended = true; return;
            }
        ...
        handleScroll() {
            ...
            if (lastStepElement && typeof lastStepElement.offsetTop === 'number' && scrollPosition >= lastStepElement.offsetTop + lastStepElement.offsetHeight) { 
        ...
        scrollToSection: sel => {
            ...
            } catch(e) { console.error("ScrollToSection error:", e); }
        },

        navigateToStep(targetStep) {
            console.log(`NAVIGATE TO STEP: Attempting to navigate to step ${targetStep}. Current: ${this.currentActiveStep}`);
            this.clearAllErrors();
            for (let i = 1; i < targetStep; i++) {
                if (!this.currentStepCompleted(i, true)) { 
                    console.log(`NAVIGATE TO STEP: Step ${i} not completed. Setting active step to ${i} and stopping navigation.`);
                    this.currentActiveStep = i;
                    this.scrollToSection(this.stepSectionsMeta[i-1]?.id || '#udheya-booking-form-panel');
                    return;
                }
            }
            console.log(`NAVIGATE TO STEP: All previous steps completed. Setting active step to ${targetStep}.`);
        ...
        currentStepCompleted(stepNumber, showErrorsIfCurrent = true) {
            console.log(`currentStepCompleted: Checking step ${stepNumber}, showErrors: ${showErrorsIfCurrent}, currentActiveStep: ${this.currentActiveStep}`);
            let isValid = false;
            ...
            console.log(`currentStepCompleted: Step ${stepNumber} validation result: ${isValid}`);
        ...
        validateAndProceedToStep(nextStep) {
            console.log(`VALIDATE AND PROCEED: To step ${nextStep}. Current active step: ${this.currentActiveStep}`);
            this.clearAllErrors();
            let isValidCurrentStep = this.currentStepCompleted(this.currentActiveStep, true);
            console.log(`VALIDATE AND PROCEED: Current step (${this.currentActiveStep}) validation result: ${isValidCurrentStep}`);

            if (isValidCurrentStep) {
                if (nextStep <= this.stepSectionsMeta.length) {
                    console.log(`VALIDATE AND PROCEED: Proceeding to step ${nextStep}.`);
                    ...
                    console.log(`VALIDATE AND PROCEED: nextStep ${nextStep} is out of bounds.`);
                }
            } else {
                console.log(`VALIDATE AND PROCEED: Current step ${this.currentActiveStep} is not valid. Staying on current step.`);
                this.$nextTick(() => { 
        ...
        validateStep1(showErrors = true) {
            console.log('VALIDATE STEP 1: showErrors =', showErrors, 'Selected animal type:', this.selectedAnimal.type);
            ...
            if (!this.selectedAnimal.type) { 
                if(showErrors) {
                    ...
                    console.log('VALIDATE STEP 1: Animal not selected, focusing on:', firstErrorRef);
            ...
            console.log('VALIDATE STEP 1: Passed.');
            return true;
        },
        validateStep2(showErrors = true) {
            console.log('VALIDATE STEP 2: showErrors =', showErrors);
            ...
            if(showErrors && firstErrorRef) { console.log('VALIDATE STEP 2: Error found, focusing on:', firstErrorRef); this.focusOnRef(firstErrorRef); }
            if(this.stepSectionsMeta[1]) this.stepSectionsMeta[1].firstFocusableErrorRef = firstErrorRef;
            console.log('VALIDATE STEP 2: Result =', isValid);
            return isValid;
        },
        validateStep3(showErrors = true) {
            console.log('VALIDATE STEP 3: showErrors =', showErrors, '(Always returns true for now)');
            if(this.stepSectionsMeta[2]) this.stepSectionsMeta[2].firstFocusableErrorRef = null; return true;
        },
        validateStep4(showErrors = true) {
            console.log('VALIDATE STEP 4: showErrors =', showErrors);
            ...
            if(showErrors && firstErrorRef) { console.log('VALIDATE STEP 4: Error found, focusing on:', firstErrorRef); this.focusOnRef(firstErrorRef); }
            if(this.stepSectionsMeta[3]) this.stepSectionsMeta[3].firstFocusableErrorRef = firstErrorRef;
            console.log('VALIDATE STEP 4: Result =', isValid);
            return isValid;
        },
        validateStep5(showErrors = true) {
            console.log('VALIDATE STEP 5: showErrors =', showErrors);
            ...
            if(!this.paymentMethod) {
                ...
                console.log('VALIDATE STEP 5: Payment method not selected.');
                return false;
            }
            if(this.stepSectionsMeta[4]) this.stepSectionsMeta[4].firstFocusableErrorRef = null;
            console.log('VALIDATE STEP 5: Passed.');
            return true;
        },

        selectAnimal(cardEl, key) {
            console.log(`SELECT ANIMAL: Key = ${key}`);
            ...
            if (!animal || !sel || !sel.value) {
                console.error("SELECT ANIMAL: Animal or select element/value missing.", animal, sel, sel?.value);
                this.setError('animal', 'select');
            ...
            if (!wp || !wp.is_active || (wp.stock != null && wp.stock <= 0)) {
                console.warn("SELECT ANIMAL: Weight/price data missing or out of stock.", wp);
            ...
            console.log("SELECT ANIMAL: Selected animal object:", JSON.parse(JSON.stringify(this.selectedAnimal)));
            this.calculateTotalPrice();
            // Ensure step 1 is now considered complete before trying to proceed
            if (this.validateStep1(false)) { // Validate silently
                this.validateAndProceedToStep(2);
            } else {
                console.warn("SELECT ANIMAL: Step 1 still not valid after selection, not proceeding.");
            }
        },
            ...
            try {
                 (this.productOptions.livestock || []).forEach(animal => {
                    ...
                    (animal.weights_prices || []).forEach(wp => {
                        ...
                        const stockText = oos ? ' - Out of Stock' : (wp.stock != null ? ` - Stock: ${wp.stock}` : '');
                        opt.textContent = `${wp.weight_range || ''} (${this.getFormattedPrice(wp.price_egp)})${stockText}`;
                    ...
                    sel.value = curVal && curAvail ? curVal : (firstAvail || (sel.options.length ? sel.options[0].value : ''));
                ...
                console.error("Error in updateAllDisplayedPrices:", e);
                this.userFriendlyApiError = "Error updating price displays."; 
            }
        },

        async validateAndSubmitBooking() {
            console.log("VALIDATE AND SUBMIT: Starting final validation...");
            this.clearAllErrors();
            ...
            console.log("VALIDATE AND SUBMIT: All steps validated.");
            ...
            if (!animal || !wpData || !wpData.is_active || (wpData.stock != null && wpData.stock <= 0)) {
                this.setError('animal', {en: `Sorry, ${this.selectedAnimal.nameEN || 'item'} is no longer available. Please reselect.`, ar: `عذراً، ${this.selectedAnimal.nameAR || 'المنتج'} لم يعد متوفراً. يرجى إعادة الاختيار.`});
            ...
            console.log("VALIDATE AND SUBMIT: Payload prepared:", JSON.stringify(payload));
            ...
            try {
                ...
                console.log("VALIDATE AND SUBMIT: Booking successful. Booking ID:", this.bookingID);
                if (wpData.stock != null && wpData.stock > 0) {
                    wpData.stock--;
                    console.log("VALIDATE AND SUBMIT: Decremented stock for", animal.value_key, wpData.weight_range, "New stock:", wpData.stock);
                    this.updateAllDisplayedPrices(); 
                }
                ...
                console.error("VALIDATE AND SUBMIT: Booking submission error:", error);
        ...
        resetAndStartOver() {
            console.log("RESET AND START OVER: Called.");
            ...
            if(this.countdownTimerInterval) clearInterval(this.countdownTimerInterval);
            // Re-initialize parts of app state, but not full data fetch if not needed
            this.initApp(); // Re-fetch all data and reset UI
...
(function() { 
    ...
    if (getQueryParam(SEEDER_QUERY_PARAM) === 'true') {
        ...
        // console.log("Ensure 'app_settings' and 'livestock_types' collections exist and 'Create Rule' is temporarily open for them."); // Commented out, assume it's handled.
        ...
        async function runInlineSeed() {
            ...
            // console.warn("SEEDER: IMPORTANT! Remember to re-secure your API rules in PocketBase if you opened them."); // Commented out for less noise now

            if (window.history.replaceState) {
                const cleanURL = window.location.protocol + "//" + window.location.host + window.location.pathname;
                window.history.replaceState({path: cleanURL}, '', cleanURL);
                // console.log("SEEDER: Cleared seed query parameter from URL."); // Commented out
            }
        }
        runInlineSeed();
    }
