document.addEventListener('alpine:init', () => {
    const initialBookingFormState = {
        ...
        selectedTimeSlot: '8 AM-9 AM', distributionChoice: 'me', paymentMethod: 'fa', // Defaulted payment
        errors: {}
    };
    ...
    Alpine.data('udheyaBooking', () => ({
        ...
        navigateToStep(targetStep) {
            ...
            for (let i = 1; i < targetStep; i++) {
                if (!this.currentStepCompleted(i, true)) { // Show errors for the first incomplete step
        ...
        currentStepCompleted(stepNumber, showErrors = true) { // `showErrors` only matters if this step is current for validation
            switch (stepNumber) {
                case 1: return this.validateStep1(showErrors && this.currentActiveStep === 1);
                case 2: return this.validateStep2(showErrors && this.currentActiveStep === 2);
                case 3: return this.validateStep3(showErrors && this.currentActiveStep === 3);
                case 4: return this.validateStep4(showErrors && this.currentActiveStep === 4);
                case 5: return this.validateStep5(showErrors && this.currentActiveStep === 5);
        ...
        validateAndProceedToStep(nextStep) {
            ...
            if (isValidCurrentStep) {
                if (nextStep <= this.stepSectionsMeta.length) {
                 ...
                 // Focus already set by individual validation function. Scroll to current step to show errors.
        ...
        validateStep1(showErrors = true) {
            ...
            if (!this.selectedAnimal.type) {
                if(showErrors) { this.setError('animal', 'select'); this.focusOnRef(this.$refs.baladiWeightSelect?.closest('.livestock-card') ? 'baladiWeightSelect' : 'barkiWeightSelect');} // Heuristic
                return false;
            }
            this.stepSectionsMeta[0].firstFocusableErrorRef = null; // Clear if valid
            return true;
        },
        ...
        validateStep3(showErrors = true) { /* Schedule: Day, Time - No specific validation, defaults exist */ return true; },
        validateStep4(showErrors = true) { // Distribution & Delivery
            ...
            if (this.distributionChoice === 'split') {
                if (!this.splitDetailsOption) setValError('splitDetails', 'select', 'distributionChoiceRadios'); // Focus on the group
        ...
        getSacrificeDayText(val) {
            const opt = document.querySelector(`#sacrifice_day_select_s3 option[value="${val}"]`); // Adjusted ID for 5-step
